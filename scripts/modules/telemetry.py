...
class scsTelemetry:
    ...
    def readable(self, time):
        ...
        return "{} {}:{}".format(
            self.days[days], str(hours).zfill(2), str(minutes).zfill(2)
        )
    ...
    def readTrailer(self, offset, count=1):
        ...
        for _i in range(count):
            ...
            data["comFloat"]["wheelSuspDeflection"], offset = self.readFloat(
                offset, count=16
            )
            ...
            data["comFloat"]["wheelLiftOffset"], offset = self.readFloat(
                offset, count=16
            )
            ...
            data["conVector"]["wheelPositionX"], offset = self.readFloat(
                offset, count=16
            )
            data["conVector"]["wheelPositionY"], offset = self.readFloat(
                offset, count=16
            )
            data["conVector"]["wheelPositionZ"], offset = self.readFloat(
                offset, count=16
            )
            ...
            data["conString"]["cargoAccessoryId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["conString"]["bodyType"], offset = self.readChar(
                offset, count=stringSize
            )
            data["conString"]["brandId"], offset = self.readChar(
                offset, count=stringSize
            )
            ...
            data["conString"]["chainType"], offset = self.readChar(
                offset, count=stringSize
            )
            data["conString"]["licensePlate"], offset = self.readChar(
                offset, count=stringSize
            )
            data["conString"]["licensePlateCountry"], offset = self.readChar(
                offset, count=stringSize
            )
            data["conString"]["licensePlateCountryId"], offset = self.readChar(
                offset, count=stringSize
            )
    ...
    def readBool(self, offset, count=1):
        if count == 1:
            ...
            for i in range(count):
                bools.append(
                    struct.unpack("?", self.mm[offset + i : offset + i + 1])[0]
                )
    ...
    def readInt(self, offset, count=1):
        if count == 1:
            ...
            for i in range(count):
                ints.append(
                    struct.unpack("i", self.mm[offset + i * 4 : offset + i * 4 + 4])[0]
                )
    ...
    def readFloat(self, offset, count=1):
        if count == 1:
            ...
            for i in range(count):
                floats.append(
                    struct.unpack("f", self.mm[offset + i * 4 : offset + i * 4 + 4])[0]
                )
    ...
    def readLong(self, offset, count=1):
        if count == 1:
            ...
            for i in range(count):
                longs.append(
                    struct.unpack("q", self.mm[offset + i * 8 : offset + i * 8 + 8])[0]
                )
    ...
    def readChar(self, offset, count):
        ...
        for i in range(count):
            try:
                newChar = struct.unpack("s", self.mm[offset + i : offset + i + 1])[
                    0
                ].decode("utf-8")
            except Exception:
                continue
    ...
    def readDouble(self, offset, count=1):
        if count == 1:
            ...
            for i in range(count):
                doubles.append(
                    struct.unpack("d", self.mm[offset + i * 8 : offset + i * 8 + 8])[0]
                )
    ...
    def update(self, trailerData=False):
        if os.name != "nt":
            ...
            self.mm = mmap.mmap(
                self.fd.fileno(), length=0, flags=mmap.MAP_SHARED, prot=mmap.PROT_READ
            )
        else:
            self.mm = mmap.mmap(0, mmapSize, mmapName)
        ...
        try:
            ...
            data["scsValues"]["telemetryVersionGameMajor"], offset = self.readInt(
                offset
            )
            data["scsValues"]["telemetryVersionGameMinor"], offset = self.readInt(
                offset
            )
            ...
            try:
                data["commonUI"]["timeRdbl"] = self.readable(
                    data["commonUI"]["timeAbs"]
                )
            except Exception:
            ...
            data["truckUI"]["truckWheelSubstance"], offset = self.readInt(
                offset, count=16
            )
            ...
            data["gameplayUI"] = {}
            data["gameplayUI"]["jobDeliveredDeliveryTime"], offset = self.readInt(
                offset
            )
            ...
            data["truckInt"]["hshifterResulting"], offset = self.readInt(
                offset, count=32
            )
            ...
            data["configFloat"]["waterTemperatureWarning"], offset = self.readFloat(
                offset
            )
            data["configFloat"]["batteryVoltageWarning"], offset = self.readFloat(
                offset
            )
            ...
            data["configFloat"]["truckWheelRadius"], offset = self.readFloat(
                offset, count=16
            )
            data["configFloat"]["gearRatiosForward"], offset = self.readFloat(
                offset, count=24
            )
            data["configFloat"]["gearRatiosReverse"], offset = self.readFloat(
                offset, count=8
            )
            ...
            data["truckFloat"]["truck_wheelSuspDeflection"], offset = self.readFloat(
                offset, count=16
            )
            data["truckFloat"]["truck_wheelVelocity"], offset = self.readFloat(
                offset, count=16
            )
            data["truckFloat"]["truck_wheelSteering"], offset = self.readFloat(
                offset, count=16
            )
            data["truckFloat"]["truck_wheelRotation"], offset = self.readFloat(
                offset, count=16
            )
            data["truckFloat"]["truck_wheelLift"], offset = self.readFloat(
                offset, count=16
            )
            data["truckFloat"]["truck_wheelLiftOffset"], offset = self.readFloat(
                offset, count=16
            )

            data["gameplayFloat"] = {}
            data["gameplayFloat"]["jobDeliveredCargoDamage"], offset = self.readFloat(
                offset
            )
            data["gameplayFloat"]["jobDeliveredDistanceKm"], offset = self.readFloat(
                offset
            )
            ...
            data["configBool"] = {}
            data["configBool"]["truckWheelSteerable"], offset = self.readBool(
                offset, count=16
            )
            data["configBool"]["truckWheelSimulated"], offset = self.readBool(
                offset, count=16
            )
            data["configBool"]["truckWheelPowered"], offset = self.readBool(
                offset, count=16
            )
            data["configBool"]["truckWheelLiftable"], offset = self.readBool(
                offset, count=16
            )
            ...
            data["truckBool"]["truck_wheelOnGround"], offset = self.readBool(
                offset, count=16
            )
            ...
            data["truckBool"]["trailerLiftAxleIndicator"], offset = self.readBool(
                offset
            )

            data["gameplayBool"] = {}
            data["gameplayBool"]["jobDeliveredAutoparkUsed"], offset = self.readBool(
                offset
            )
            data["gameplayBool"]["jobDeliveredAutoloadUsed"], offset = self.readBool(
                offset
            )
            ...
            data["configVector"]["truckWheelPositionX"], offset = self.readFloat(
                offset, count=16
            )
            data["configVector"]["truckWheelPositionY"], offset = self.readFloat(
                offset, count=16
            )
            data["configVector"]["truckWheelPositionZ"], offset = self.readFloat(
                offset, count=16
            )
            ...
            data["headPlacement"]["cabinOffsetrotationX"], offset = self.readFloat(
                offset
            )
            data["headPlacement"]["cabinOffsetrotationY"], offset = self.readFloat(
                offset
            )
            data["headPlacement"]["cabinOffsetrotationZ"], offset = self.readFloat(
                offset
            )
            ...
            data["headPlacement"]["headOffsetrotationX"], offset = self.readFloat(
                offset
            )
            data["headPlacement"]["headOffsetrotationY"], offset = self.readFloat(
                offset
            )
            data["headPlacement"]["headOffsetrotationZ"], offset = self.readFloat(
                offset
            )
            ...
            data["configString"] = {}
            data["configString"]["truckBrandId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["truckBrand"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["truckId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["truckName"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["cargoId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["cargo"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["cityDstId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["cityDst"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["compDstId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["compDst"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["citySrcId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["citySrc"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["compSrcId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["compSrc"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["shifterType"], offset = self.readChar(
                offset, count=16
            )
            data["configString"]["truckLicensePlate"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["truckLicensePlateCountryId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["truckLicensePlateCountry"], offset = self.readChar(
                offset, count=stringSize
            )
            data["configString"]["jobMarket"], offset = self.readChar(offset, count=32)

            data["gameplayString"] = {}
            data["gameplayString"]["fineOffence"], offset = self.readChar(
                offset, count=32
            )
            data["gameplayString"]["ferrySourceName"], offset = self.readChar(
                offset, count=stringSize
            )
            data["gameplayString"]["ferryTargetName"], offset = self.readChar(
                offset, count=stringSize
            )
            data["gameplayString"]["ferrySourceId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["gameplayString"]["ferryTargetId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["gameplayString"]["trainSourceName"], offset = self.readChar(
                offset, count=stringSize
            )
            data["gameplayString"]["trainTargetName"], offset = self.readChar(
                offset, count=stringSize
            )
            data["gameplayString"]["trainSourceId"], offset = self.readChar(
                offset, count=stringSize
            )
            data["gameplayString"]["trainTargetId"], offset = self.readChar(
                offset, count=stringSize
            )
            ...
            data["gameplayLongLong"]["jobCancelledPenalty"], offset = self.readLongLong(
                offset
            )
            data["gameplayLongLong"]["jobDeliveredRevenue"], offset = self.readLongLong(
                offset
            )
            data["gameplayLongLong"]["fineAmount"], offset = self.readLongLong(offset)
            data["gameplayLongLong"]["tollgatePayAmount"], offset = self.readLongLong(
                offset
            )
            data["gameplayLongLong"]["ferryPayAmount"], offset = self.readLongLong(
                offset
            )
            data["gameplayLongLong"]["trainPayAmount"], offset = self.readLongLong(
                offset
            )
            ...
            data["substances"], offset = self.readStringArray(
                offset, count=substanceSize, stringSize=stringSize
            )
